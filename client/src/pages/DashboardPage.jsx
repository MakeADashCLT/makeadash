...
export default function DashboardPage({ onLogout }) {
  ...
  async function fetchWeatherForWidget(widgetId, city) {
    ...
    try {
      const response = await fetch(
        `http://localhost:5002/api/weather?city=${encodeURIComponent(city)}`
      );
  ...
  async function fetchSteamForWidget(widgetId, query = '') {
    ...
    try {
      const response = await fetch(
        `http://localhost:5002/api/steam/featured?q=${encodeURIComponent(query)}`
      );
  ...
  async function fetchAniListForWidget(widgetId, mode = 'today') {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:5002/api/anilist/${endpoint}`);
      const data = await response.json();
  ...
  async function fetchGithubForWidget(widgetId, username) {
    ...
    try {
      const res = await fetch(
        `http://localhost:5002/api/github/user?username=${encodeURIComponent(username)}`,
        { credentials: 'include' }
      );
